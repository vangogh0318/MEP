...
class DataDownloader(ABC):
    ...
    def tokenize(self):
        ...
        jsonl_filepath = ",".join(
            ...
            [os.path.join(parent_folder, os.path.basename(file)) for file in os.listdir(parent_folder)]
        )
        #print("folder:%s, jsonl_filepath:%s" % (parent_folder, jsonl_filepath))
    ...
    def prepare(self):
        print("prepare")
        print("dir:%s, name:%s" % (self.base_dir, self.name))
        self.tokenize()
        if not self.exists():
            self.download()
            self.tokenize()
...
class PG19(DataDownloader):
    name = "pg19"
    urls = ["https://xxx.org/pg19.zip"]
