...
class ParallelSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        if self.pos_emb == "alibi":
            print('pos_emb alibi, invoke Alibi function')
        ...
        if rotary:
            ...
            if neox_args.alibi_scaling == 0:
                print('rotary_emb = RotaryEmbedding')
                self.rotary_emb = RotaryEmbedding(
                    dim, base=neox_args.rotary_emb_base, precision=neox_args.params_dtype
                )
            elif neox_args.alibi_scaling == 1:
                print('rotary_emb = LlamaPartNTKScaledRotaryEmbedding')
                print('seq_len=%s' % (neox_args.seq_length_val))
                self.rotary_emb = LlamaPartNTKScaledRotaryEmbedding(
                    dim, 
                    max_position_embeddings=neox_args.seq_length_val,
                    scale=1,
                    original_max_position_embeddings=512
                )
            else:
                print('rotary_emb = LlamaYaRNScaledRotaryEmbedding')
                print('seq_len=%s' % (neox_args.seq_length_val))
    ...
    def attention(
        ...
        if exists(self.rpe):
            if self.pos_emb.startswith("kerple") or self.pos_emb.startswith("snope") :
                #print('kerple, invokde self.rpe')
                attention_scores = self.rpe(attention_scores)
            else:
                #print('t5, invokde self.rpe')
                rpe = self.rpe(query_layer.size(0), key_layer.size(0))
                attention_scores += rpe  # [1, np, sq, sk]

        if self.pos_emb == "alibi":
            #print('pos_emb alibi, invoke alibi_embed function')
    ...
    def forward(self, hidden_states, attention_mask, layer_past=None):
        ...
        if exists(self.rotary_emb):
            if exists(self.rotary_ndims):
                ...
                # full rotary
                #print('rotary, invokd apply_rotary_fn')
