...
from megatron.mpu import ParallelRelativePositionBias, ParallelKerplePower, ParallelKerpleLog, ParallelSNOPE, ParallelSNOPEKerpleLog
...
class GPT2ModelPipe(PipelineModule, torch.nn.Module):
    ...
    def init_specs(self):
        ...
        # snope
        if self.neox_args.pos_emb.startswith("snope_norm"):
            print('pos_emb == snope')
            rpe_emb = ParallelSNOPE(
                neox_args=self.neox_args,
            )

        # snope_kerplelog merge
        if self.neox_args.pos_emb.startswith("snope_kerplelog"):
            print('pos_emb == kerple_log and snope MKL')
            rpe_emb = ParallelSNOPEKerpleLog(
                neox_args=self.neox_args,
            )
       
        # Kernelized Alibi positional embedding
        if self.neox_args.pos_emb.startswith("kerple_power"):
            print('pos_emb == kerple_power')
            ...
            if layer_type in ["gmlp", "amlp"]:
                ...
                self.specs.append(
                    LayerSpec(
                        ...
                        output_layer_init_method=self.output_layer_init_method,
                        layer_number=i,
                        rpe=rpe_emb if self.neox_args.pos_emb.startswith(tuple(["snope", "rpe", "kerple"])) else None,
