...
def pretrain(neox_args):
    ...
    if neox_args.do_valid:
        ...
        print("neox_args.do_valid:{}".format(neox_args.do_valid))
...
def setup_model_and_optimizer(neox_args, use_cache=False, iteration=None):
    ...
    if neox_args.load is not None:
        ...
        for mname, mvalue in model_dict.items():
            if "bias" in mname:
                print_rank_0("model name:{}, value:{}".format(mname, mvalue))
        import sys
        sys.exit(0)
        '''
    else:
...
def train_step_pipe(neox_args, timers, model, data_iterator):
    ...
    for k, v in model.module.named_modules():
        if 'attention.rpe' in k and (neox_args.pos_emb.startswith('kerple') or neox_args.pos_emb.startswith('snope')):
            stats = v.stats()
            break
